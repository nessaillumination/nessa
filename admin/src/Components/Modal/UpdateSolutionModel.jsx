...
import { getProduct, querySingleSolutions, updateSolutionData, uploadFile } from '../../service/apiService'
import { toast } from 'react-hot-toast'

const UpdateSolutionModel = ({ open, onClose, token, solutionId }) => {
    ...
    // Fetch solution details when solutionId is passed
    useEffect(() => {
        ...
        const fetchSolution = async () => {
            ...
            try {
                ...
                setFormData(fetchedSolution.data)
                setUploadsComplete({ thumbnail: fetchedSolution.data?.thumbnail, solutionImageUrl: fetchedSolution.data?.solutionImageUrl })
            } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await updateSolutionData(solutionId, updatedFormData)
            if (response.success) {
                toast.success('Solution updated successfully')
                onClose()
                fetchProducts()
            } else {
                toast.error('Solution update failed')
            }
        } catch (error) {
    ...
    return (
        <Modal
            ...
            <motion.div
                ...
                <h2 className="text-3xl font-semibold text-gray-800 mb-6">Update Solution</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="flex justify-end space-x-3 mt-6">
                        ...
                        <button
                            ...
                            disabled={loading}>
                            {loading ? 'Update...' : 'Update Solution'}
                        </button>
                    </div>
                </form>
            </motion.div>
...
export default UpdateSolutionModel
